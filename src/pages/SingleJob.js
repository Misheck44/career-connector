import Upload from './Upload.js'; // Assuming Upload.js is in the same directory
...
const SingleJob = () => {
  ...
  const applyForAJob = async () => {
    if (Upload.isUploadSuccessful) { // Access prop from Upload
        uploadSuccess();
      } else {
        // Handle upload failure scenario (e.g., display an error message)
            ...
            >
                  ...
                  <Card sx={{ bgcolor: palette.primary.white }} >
                    <CardContent>
                      ...
                      <Typography variant="body2">
                        <Box component="span" sx={{ fontWeight: 700 }}>Company</Box>:{singleJob && singleJob.company}
                      </Typography>
                      <Typography variant="body2">
              ...
              <Box sx={{ flex: 1, p: 2 }}>
                <Card sx={{ p: 2, bgcolor: palette.primary.white }}>
                  <Upload onUploadSuccess={uploadSuccess} /> {/* Pass callback function */}
                  <Button onClick={applyForAJob} sx={{ fontSize: "13px" }} variant='contained'>Apply for this Internship </Button>
                </Card>
              </Box>
